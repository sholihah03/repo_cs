    ...
    public function login(Request $request)
{
    ...
    if (Auth::guard($accountType)->attempt($credentials)) {
        ...
        switch ($accountType) {
            case 'direktur':
                return redirect()->route('dashboardDirektur'); // Ubah sesuai dengan rute dashboard direktur
            case 'manager':
                return redirect()->route('dashboardRekap'); // Ubah sesuai dengan rute dashboard manajer
            case 'karyawan':
                return redirect()->route('dashboardKaryawan'); // Ubah sesuai dengan rute dashboard karyawan
            case 'cs':
                return redirect()->route('dashboardcs'); // Ubah sesuai dengan rute dashboard CS
            case 'advertiser':
                return redirect()->route('dashboardAdvertiser'); // Ubah sesuai dengan rute dashboard advertiser
            default:
